...
{% if image_id %}
    <a href="{% url blog_article id=id slug=m.rsc[id].slug %}">
        {% image m.rsc[id].media[1] mediaclass="roadmap_summary" %}
   </a>
{% endif %}
<p>
    <a class="btn btn-xs btn-default pull-right" href="{% url blog_article id=id slug=m.rsc[id].slug %}">{_ Read more _}</a>
</p>
{% endwith %}