<div>
    ...
    {% if m.rsc[id].is_released %}
        <span class="label label-success">{{ m.rsc[id].release_date|date:"F Y"}} {_ Released _}</span>
    {% elif m.rsc[id].is_planned %}
        <span class="label label-info">{{ m.rsc[id].planned_date_end|date:"F Y"}} {_ Planned _}</span>
    {% endif %}
